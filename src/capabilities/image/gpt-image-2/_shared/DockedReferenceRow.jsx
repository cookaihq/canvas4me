 ...
 *
 * 与 V1 右侧面板的 ReferenceImageUploader (多行网格 + 悬停按钮) 是两种独立形态:
 * 这里是 52×52 缩略图横排, X 按钮外露在右上角, 末尾可选 + 按钮.
 *
 * items 由父组件合并 (端口连入):
 ...
 *   - + → onInsertToken(globalIndex) 往 prompt 插入 @图像N
 *   - 查看 → 弹出大图预览 (antd Image.preview)
 *   - x → onDelete(item) 断开连线 (uploading 项视为"取消上传", 父组件同时删占位节点)
 */
export default function DockedReferenceRow({
  items = [],
  max = 10,
  showAddButton = true,
